...
namespace ShadowMUD.Cache
{
    ...
    public class LRUCache<TItemType> where TItemType : class
    {
        ...
        private class Index<TKeyType> : IIndex<TKeyType>, IIndex
        {
            ...
            {
                RWLock.GetWriteLock(_lock, LockTimeout, delegate { _index.Clear(); return true; });
            }
            ...
            public bool AddItem(INode item)
            {
                ...
                return RWLock.GetWriteLock(_lock, LockTimeout,
                    delegate
                    {
                        var isDup = _index.ContainsKey(key);
            ...
            public int RebuildIndex()
            {
                lock (_owner._lifeSpan)
                    return RWLock.GetWriteLock(_lock, LockTimeout,
                        ...
                        {
                            _index.Clear();
                            foreach (var item in _owner._lifeSpan)
                                AddItem(item);
                            return _index.Count;
                        });
            }
        ...
        private class LifespanMgr : IEnumerable<INode>
        {
            ...
            {
                _owner = owner;
                var maxMs = Math.Min((int) maxAge.TotalMilliseconds, 12*60*60*1000); // max = 12 hours
                _minAge = minAge;
                ...
                for (var loop = Size - 1; loop >= 0; --loop)
                    _bags[loop] = new AgeBag();
                OpenCurrentBag(DateTime.Now, 0);
            }

            #region IEnumerable<LRUCache<ItemType>.INode> Members

            /// <summary>Create item enumerator</summary>
            public IEnumerator<INode> GetEnumerator()
            {
                for (var bagNumber = _current; bagNumber >= _oldest; --bagNumber)
                {
                    var bag = _bags[bagNumber];
                    // if bag.first == null then bag is empty or being cleaned up, so skip it!
                    for (var node = bag.First; node != null && bag.First != null; node = node.Next)
                        if (node.Value != null)
                            yield return node;
                }
            }
            ...
            private void CleanUp(DateTime now)
            {
                lock (this)
                {
                    ...
                    var maxAge = now.Subtract(_maxAge);
                    var minAge = now.Subtract(_minAge);
                    var itemsToRemove = _owner._curCount - _owner._capacity;
                    var bag = _bags[_oldest%Size];
            ...
            private void CheckIndexValid()
            {
                ...
                foreach (var keyValue in _owner._indexList)
                {
            ...
            private void OpenCurrentBag(DateTime now, int bagNumber)
            {
                lock (this)
                {
                    ...
                    var currentBag = _bags[(_current = bagNumber)%Size];
        ...
        protected IsCacheValid IsValid;
        ...
        {
            IIndex index;
            return (_indexList.TryGetValue(indexName, out index) ? index as IIndex<TKeyType> : null);
        }

        /// <summary>Retrieve a object by index name / key</summary>
        public TItemType GetValue<TKeyType>(String indexName, TKeyType key)
        {
            var index = GetIndex<TKeyType>(indexName);
        ...
        private INode Add(TItemType item)
        {
            ...
            foreach (var keyValue in _indexList)
            ...
            var isDupl = (node != null && node.Value == item);
